...
function loadTests() {
    ...
    // This requires checking student results for these tests
    const allUsersData = JSON.parse(localStorage.getItem('placeme_users') || '{"students":[]}');
    const students = allUsersData.students || [];
    let totalAttempts = 0;
    let totalPassed = 0;
    
    userTests.forEach(test => {
        students.forEach(student => {
            const result = (student.testsCompleted || []).find(rt => rt.testId == test.id || rt.testName === test.name);
            if (result) {
                totalAttempts++;
