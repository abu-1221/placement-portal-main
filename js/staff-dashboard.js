...
function initCreateTestForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            // Save to Backend API
            if (window.DB) {
                await window.DB.createTest(test);
...
async function loadTests() {
    ...
    // Fetch from API
    const tests = window.DB ? await window.DB.getTests() : [];
    ...
    const totalParticipants = userTests.reduce((sum, t) => sum + (t.participants || 0), 0);
    
    // Calculate global pass rate for this staff's tests
    let totalAttempts = 0;
    let totalPassed = 0;
    
    userTests.forEach(test => {
        const testResults = allResults.filter(r => r.testId == test.id || r.testName === test.name);
...
async function deleteTest(id) {
    ...
    try {
        if (window.DB) {
            ...
            showNotification('Deleted', 'Test removed successfully.', 'success');
            // Refresh stats
            loadTests();
        }
    } catch (err) {
        console.error(err);
        showNotification('Error', 'Failed to delete test: ' + err.message, 'error');
    }
}
...
async function loadStudents() {
    ...
    const depts = new Set(students.map(s => {
        // Handle nested details if stored that way, or direct attributes
    ...
    students.forEach(student => {
        ...
        const row = `
            <tr data-student-id="${student.username}">
                ...
                <td>${details.year ? details.year + getSuffix(details.year) + ' Year' : '-'}</td>
                <td>${details.className || '-'}</td>
                <td>${details.batch || '-'}</td>
                <td>${student.email || details.email || '-'}</td>
                <td>${student.phoneNumber || details.phoneNumber || '-'}</td>
...
async function initAnalytics() {
    ...
    // Get tests created by this staff from API
    const tests = window.DB ? await window.DB.getTests() : [];
    const userTests = tests.filter(t => t.createdBy === user.username);
...
async function loadTestAnalytics(testId) {
    ...
    const testResults = allResults.filter(r => r.testId == testId || r.testName === test.name);
    
    const results = testResults.map(tr => {
        const student = students.find(s => s.username === tr.username);
        return {
            student: student ? student.name : tr.username,
            score: tr.score,
            status: tr.status,
            date: tr.createdAt
        };
