...
function initCharts() {
    const user = JSON.parse(sessionStorage.getItem('user') || '{}');
    if (user.type !== 'student') return; // Guard clause

    const tests = user.testsCompleted || [];
    
    // Analytics charts are now handled by analytics.js
    // We remove the old "No data" wipe logic to let analytics.js render empty charts if needed.
    
    /*
    if (tests.length === 0) {
        // Removed to allow empty charts
        return;
    }
    */
...
function confirmStartTest(testId) {
    if (confirm("Are you sure you want to start this test? Once started, the timer will begin.")) {
...
function startInternalTest(testId) {
    ...
    if (!test) { alert("Error loading test."); return; }
    ...
    activeTestInterval = setInterval(() => {
        ...
        const s = secs % 60;
        document.getElementById('internalTimeRemaining').textContent = `Time Remaining: ${m}:${s.toString().padStart(2, '0')}`;
        ...
        <div class="question-card" style="background: rgba(255,255,255,0.05); padding: 2rem; border-radius: 1rem; border: 1px solid rgba(255,255,255,0.1);">
            ...
            <div class="options-list" style="display:flex; flex-direction:column; gap:1rem;">
                ${q.options.map((opt, i) => {
                    ...
                    const isChecked = activeTestAnswers[index] === letter ? 'checked' : '';
                    const isSelectedClass = activeTestAnswers[index] === letter ? 'background: rgba(102,126,234,0.2); border-color: #667eea;' : '';
...
function finishInternalTest(test) {
    ...
    const storedUsers = JSON.parse(localStorage.getItem('placeme_users'));
    const sIdx = storedUsers.students.findIndex(s => s.username === user.username);
    if (sIdx !== -1) {
        if (!storedUsers.students[sIdx].testsCompleted) storedUsers.students[sIdx].testsCompleted = [];
...
function loadCompletedTests() {
    ...
    if (completedTests.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 2rem;">No tests attended yet</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = completedTests.map((test, index) => {
        const formattedDate = new Date(test.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
        ...
        questions.forEach((q, i) => {
            ...
            if (q.options && Array.isArray(q.options)) {
                ...
                q.options.forEach((opt, optIdx) => {
                     ...
                     let itemStyle = 'padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.85rem;';
                     
                     if (char === correctAns && char === userAns) {
                         // Correct user choice
                         itemStyle += 'background: rgba(16, 185, 129, 0.2); color: #10b981; border: 1px solid #10b981;';
                     } else if (char === correctAns) {
                         // User missed this correct one
                         itemStyle += 'background: rgba(16, 185, 129, 0.1); color: #10b981; border: 1px dashed #10b981;';
                     } else if (char === userAns) {
                         // User selected wrong one
                         itemStyle += 'background: rgba(239, 68, 68, 0.2); color: #ef4444; border: 1px solid #ef4444;';
                     } else {
                        // Neutral
                         itemStyle += 'color: var(--gray-400); border: 1px solid transparent;';
                     }
